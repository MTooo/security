...
sm2:
  sdk:
    ...
    sm2-public-key:  "04ecc36a8b58afceda42755c86f42cd5692eeb91aa27bd8082b9b8c7320ab0cc7cd9..."
    server-id: my-server
    redis-session-store: true		   # 建议开启 redis存储 不然就是存在内存中的单机
    redis-key-prefix: sm2-server		   # 建议开启 redis存储sm4密钥时的redis key前缀
    local-secret-key: sm4密钥			  # 建议扩展项 作为redis中的密钥再次加密
...
sm2:
  sdk:
    ...
    server-id: my-client-server        # 如果服务端也要调客户端的话才需要
    redis-session-store: true		   # 建议开启 redis存储 不然就是存在内存中的单机
    redis-key-prefix: sm2-client		   # 建议开启 redis存储sm4密钥时的redis key前缀
    local-secret-key: sm4密钥			  # 建议扩展项 作为redis中的密钥再次加密
