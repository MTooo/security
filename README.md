...
SDK 使用 SLF4J 打印日志，通过 Spring Boot 标准方式控制级别：

```yaml
logging:
  level:
    io.github.mtooo: INFO      # SDK 整体日志级别（默认 INFO）
```
...
logging:
  level:
    io.github.mtooo.starter.Sm2HandshakeController: WARN   # 握手请求
    io.github.mtooo.starter.Sm2ServerInterceptor: WARN     # 安全拦截器
    io.github.mtooo.client: DEBUG                           # HTTP 客户端
    io.github.mtooo.core: WARN                              # 加解密核心
```

生产环境建议：握手 Controller 和拦截器设 `WARN`，只保留异常和关键事件。
